...
class ProposalLayer(KE.Layer):
    ...
    def call(self, inputs):
        ...
        def nms(boxes, scores):
            ...
            indices, _ = tf.raw_ops.NonMaxSuppressionV4(
                boxes=boxes, scores=scores, max_output_size=self.proposal_count,
                iou_threshold=self.nms_threshold, score_threshold=float('-inf'),
                pad_to_max_output_size=False, name="rpn_non_max_suppression")
...
class PyramidROIAlign(KE.Layer):
    ...
    def call(self, inputs):
        ...
        for i, level in enumerate(range(2, 6)):
            ...
            pooled.append(tf.raw_ops.CropAndResize(
                image=feature_maps[i], boxes=level_boxes, box_ind=box_indices, crop_size=self.pool_shape,
                method="bilinear"))
...
def detection_targets_graph(proposals, gt_class_ids, gt_boxes, gt_masks, config):
    ...
    masks = tf.raw_ops.CropAndResize(image=tf.cast(roi_masks, tf.float32), boxes=boxes,
                                     box_ind=box_ids,
...
def refine_detections_graph(rois, probs, deltas, window, config):
    ...
    if config.DETECTION_MIN_CONFIDENCE:
        ...
        a = tf.expand_dims(keep, 1)
        b = tf.expand_dims(conf_keep, 1)
    ...
    def nms_keep_map(class_id):
        ...
        class_keep, _ = tf.raw_ops.NonMaxSuppressionV4(
    ...
    nms_keep = tf.map_fn(
        nms_keep_map, unique_pre_nms_class_ids, dtype=tf.int64, fn_output_signature=tf.TensorSpec((100,), dtype=tf.int64))
...
# Preprocessing Layer
############################################################
class PreprocessLayer(KE.Layer):
    def __init__(self,
                 min_dim=512,
                 max_dim=512,
                 min_scale=0,
        ...
        self.mode = mode
        self.scale = tf.cast(scale,dtype=tf.float64)
        self.padding = padding
        self.crop = crop
        self.scale_cond = tf.cast(scale_cond,dtype=tf.float64)
    ...
    def call(self,image_in):
        input_image = image_in
        input_image = tf.cast(input_image[0],dtype=tf.float32)
        image_shape = tf.shape(input_image)
        h = tf.cast(image_shape[0],dtype=tf.float64)
        w = tf.cast(image_shape[1],dtype=tf.float64)
        ...
        if min_dim:
            ...
            scale = tf.math.maximum(scale, min_dim / tf.math.minimum(h, w))
        ...
        if max_dim and mode == "square":
            ...
            image_max = tf.cast(image_max,dtype=tf.float64)
            
            scaled_dim = tf.math.round(image_max * scale)
            
            scale = tf.cond(scaled_dim > max_dim, lambda: max_dim / image_max, lambda: scale)
                
        # Resize image using bilinear interpolation
        input_image = tf.cond(scale != scale_cond, lambda: tf.image.resize(
            input_image, [tf.math.round(h * scale), tf.math.round(w * scale)], method=tf.image.ResizeMethod.BILINEAR), 
                              lambda: input_image)
         # Need padding or cropping?
        
        if mode == "square":
            new_image_shape = tf.shape(input_image)
            h = tf.cast(new_image_shape[0],dtype=tf.int64)
            w = tf.cast(new_image_shape[1],dtype=tf.int64)
            input_image = tf.image.resize_with_pad(
                input_image, 512, 512, method=tf.image.ResizeMethod.BILINEAR,
                antialias=False
            )
            top_pad = (max_dim - h) // 2
        ...
        molded_img = tf.expand_dims(input_image,axis=0)        
        molded_img.set_shape([image_in.shape[0],512,512,image_in.shape[-1]])
        
        
        MEAN_PIXEL = np.array([123.7, 116.8, 103.9])
        molded_image =  tf.cast(molded_img, tf.float32) - MEAN_PIXEL 
        
        
        NUM_CLASSES = 4
        active_class_ids = tf.zeros(NUM_CLASSES, dtype=tf.dtypes.int32)
        a = tf.convert_to_tensor(0)
        molded_image_shape = tf.shape(molded_image[0])

        image_meta = ([a] 
            +[image_shape[0], image_shape[1], image_shape[2]]
            + [molded_image_shape[0], molded_image_shape[1], molded_image_shape[2]]
            + [window[0], window[1], window[2], window[3]]
            + [scale]
            + [active_class_ids[0], active_class_ids[1], active_class_ids[2], active_class_ids[3],]
            ) 
        image_meta = tf.convert_to_tensor([image_meta], dtype=tf.float64)
        windows = ([window])
        windows = tf.convert_to_tensor(windows)
        return molded_image, image_meta, windows

############################################################
# Custom Layer for Anchors
############################################################
...
class MaskRCNN(object):
    ...
    def build(self, mode, config):
        ...
        # Preprocessing
        img_inputs = KL.Input(shape=[None, None, 3],name="input_dynamic_image")
        x = PreprocessLayer(mode="square")(img_inputs)
...
#             shape=[config.IMAGE_SHAPE[0], config.IMAGE_SHAPE[1], config.IMAGE_SHAPE[2]], name="input_image")
        ...
        input_image_meta = x[1]
        
#         KL.Input(shape=[config.IMAGE_META_SIZE],
#                                     name="input_image_meta")


        # input_image_meta = ConstLayer(inference_anchors, name="generated_anchors")(input_image)
        if mode == "training":
            # RPN GT
...
#             input_anchors = KL.Input(shape=[65472, 4], name="input_anchors")
            param_anchors = self.get_anchors(config.IMAGE_SHAPE)
            inference_anchors = np.broadcast_to(param_anchors, (config.BATCH_SIZE,) + param_anchors.shape)
        ...
        if mode == "training":
            ...
            anchors = ConstLayer(inference_anchors, name="generated_anchors")(input_image)
